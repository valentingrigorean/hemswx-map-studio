import{aD as o,ax as pe,as as T,kF as ue,rM as fe,kD as ge,hH as _e,dH as Y,cP as Z,Fp as me,Fm as Pe,FS as ve,Dw as f,EG as w,Fr as be,Dx as z,Du as r,d3 as I,uS as K,Fo as we,Eo as S,ep as $,f8 as F,FT as De,mp as Q,EE as ee,E3 as C,rJ as Ve,dI as m,dJ as P,FU as te,rK as xe,kz as ie,f7 as Ee,d2 as ye,fm as Le,eo as Se,at as ne,e7 as se,f5 as Ce,rG as Me,Dy as ae,Dz as re,j as le,Fv as Ae,Dm as $e,dG as N,fn as U,Fn as Re,uY as Te,E1 as Oe,EO as Ue,Dt as W,f_ as Ie,FV as qe,_ as u,DN as V,Fw as je,Fx as H,FW as ze,lR as Fe,FX as R,FY as Ne,fh as We,tk as He,fj as Be,fk as Ge,fl as ke,fo as Xe,d as M,e as Je,FZ as Ye,fT as B}from"./index-BhvrJzyQ.js";import{t as Ze}from"./VisualElement-MSRBM9ic.js";let G=class{constructor(e){this._renderCoordsHelper=e,this._origin=o(),this._dirty=!1,this._count=0}set vertices(e){const n=pe(3*e.length);let i=0;for(const s of e)n[i++]=s[0],n[i++]=s[1],n[i++]=s[2];this.buffers=[n]}set buffers(e){if(this._buffers=e,this._buffers.length>0){const n=this._buffers[0],i=3*Math.floor(n.length/3/2);T(this._origin,n[i],n[i+1],n[i+2])}else T(this._origin,0,0,0);this._dirty=!0}get origin(){return this._origin}draw(e){const n=this._ensureVAO(e);n!=null&&(e.bindVAO(n),e.drawArrays(ue.TRIANGLES,0,this._count))}dispose(){this._vao!=null&&this._vao.dispose()}get _layout(){return this._renderCoordsHelper.viewingMode===1?Qe:et}_ensureVAO(e){return this._buffers==null?null:(this._vao??(this._vao=this._createVAO(e,this._buffers)),this._ensureVertexData(this._vao,this._buffers),this._vao)}_createVAO(e,n){const i=this._createDataBuffer(n);return this._dirty=!1,new fe(e,new ge(e,_e(this._layout),i))}_ensureVertexData(e,n){if(!this._dirty)return;const i=this._createDataBuffer(n);e.buffer()?.setData(i),this._dirty=!1}_createDataBuffer(e){const n=e.reduce((g,c)=>g+k(c),0);this._count=n;const i=this._layout.createBuffer(n),s=this._origin;let a=0,l=0;const d="startUp"in i?this._setUpVectors.bind(this,i):void 0;for(const g of e){for(let c=0;c<g.length;c+=3){const L=T(X,g[c],g[c+1],g[c+2]);c===0?l=this._renderCoordsHelper.getAltitude(L):this._renderCoordsHelper.setAltitude(L,l);const _=a+2*c;d?.(c,_,g,L);const j=Y(X,L,s);if(c<g.length-3){for(let v=0;v<6;v++)i.start.setVec(_+v,j);i.extrude.setValues(_,0,-1),i.extrude.setValues(_+1,1,-1),i.extrude.setValues(_+2,1,1),i.extrude.setValues(_+3,0,-1),i.extrude.setValues(_+4,1,1),i.extrude.setValues(_+5,0,1)}if(c>0)for(let v=-6;v<0;v++)i.end.setVec(_+v,j)}a+=k(g)}return i.buffer}_setUpVectors(e,n,i,s,a){const l=this._renderCoordsHelper.worldUpAtPosition(a,Ke);if(n<s.length-3)for(let d=0;d<6;d++)e.startUp.setVec(i+d,l);if(n>0)for(let d=-6;d<0;d++)e.endUp.setVec(i+d,l)}};function k(t){return 3*(2*(t.length/3-1))}const Ke=o(),X=o(),Qe=Z().vec3f("start").vec3f("end").vec2f("extrude").vec3f("startUp").vec3f("endUp"),et=Z().vec3f("start").vec3f("end").vec2f("extrude");function oe(t,e){const n=t.fragment;n.include(me),t.include(Pe),n.include(ve),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new be("depthMap",i=>i.depth?.attachment),new z("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
...
return abs(dist);
}`),t.spherical){const i=(a,l,d)=>m(a,l.heightManifoldTarget,d.camera.viewMatrix),s=(a,l)=>m(a,[0,0,0],l.camera.viewMatrix);n.uniforms.add(new S("heightManifoldOrigin",(a,l)=>(i(p,a,l),s(D,l),Y(D,D,p),$(h,D),h[3]=F(D),h)),new De("globalOrigin",a=>s(p,a)),new f("cosSphericalAngleThreshold",(a,l)=>1-Math.max(2,Q(l.camera.eye,a.heightManifoldTarget)*l.camera.perRenderPixelRatio)/F(a.heightManifoldTarget))),n.code.add(r`float globeDistancePixels(float posInGlobalOriginLength) {
...
vec4 color = vec4(0.0);`),t.heightManifoldEnabled){n.uniforms.add(new C("angleCutoff",s=>A(s)),new S("heightPlane",(s,a)=>he(s.heightManifoldTarget,s.renderCoordsHelper.worldUpAtPosition(s.heightManifoldTarget,p),a.camera.viewMatrix)));const i=t.spherical?r`normalize(globalOrigin - pos)`:r`heightPlane.xyz`;n.main.add(r`
      ...
      color = max(color, heightManifoldColor * heightManifoldAlpha);`)}return t.pointDistanceEnabled&&(n.uniforms.add(new C("angleCutoff",i=>A(i)),new S("pointDistanceSphere",(i,s)=>Ve(tt(i,s)))),n.main.add(r`float pointDistanceSphereDistance = sphereDistancePixels(pointDistanceSphere, pos);
...
color = max(color, intersectsLineColor * intersectsLineAlpha);
    ...
    ${W(s,r`vec3 startUp = vec3(0, 0, 1);`)}
    ${W(s,r`vec3 endUp = vec3(0, 0, 1);`)}
